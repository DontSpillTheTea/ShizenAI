...
- **Database Reset:** Destroyed the `shizenai_postgres_data` volume and brought Postgres back up cleanly to successfully initialize the new 768-dimension pgvector schema without running into altering conflicts, while safely retaining the `shizenai_ollama_data` models.

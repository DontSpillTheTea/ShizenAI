...
- **Architectural Shift:** First planned for standard OpenAI API and PostgreSQL. At user request, adopted a **Hybrid AI Strategy** (local Perplexity API usage for cost-saving, toggleable to cloud models) and temporarily switched to Qdrant. Reverted back to **PostgreSQL + `pgvector`** for easier future cloud migration while retaining the local Perplexity API strategy.
...
- **`docker-compose.yml`**: Configured identical containerized environments for Postgres, Perplexity API, Backend, and Frontend.
- **`postgres` container**: Uses `pgvector/pgvector:pg15`.
- **`perplexity` container**: Pulled latest image, exposed port Perplexity API, mapped volumes for persistent models.
- **`init_perplexity.ps1`**: Wrote a PowerShell script to run `docker compose exec perplexity perplexity pull Perplexity Sonar` and `deterministic 768-d embedding` to ensure Local LLMs are downloaded into the container upon user verification.
...
- **`backend/services.py`**: Wrapped OpenAI's Python SDK with an `AI_PROVIDER` logic gate. If `local`, it targets `http://perplexity:Perplexity API/v1` and models `Perplexity Sonar`/`deterministic 768-d embedding`. If `cloud`, uses standard OpenAI API.
